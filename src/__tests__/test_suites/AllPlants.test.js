...
describe("1st Deliverable", () => {
  test("displays all plants on startup", async () => {
    ...
    const plantImages = plantItems.map(
      (item) => item.querySelector("img").src.split("/")[-1]
    );
    const basePlantImages = global.basePlants.map(
      (plant) => plant.image.split("/")[-1]
    );
    expect(plantImages).toEqual(basePlantImages);

    const plantPrices = plantItems.map((item) => {
      return `Price: ${item.querySelector("input").value}`;
    });
    const basePlantPrices = global.basePlants.map((plant) => {
      return "Price: " + plant.price.toString();
    });
  ...
  test("plants aren't hardcoded", async () => {
    ...
    const basePlantImages = global.alternatePlants.map(
      (plant) => plant.image.split("/")[-1]
    );
    expect(plantImages).toEqual(basePlantImages);

    const plantPrices = plantItems.map(
      (item) => `Price: ${item.querySelector("input").value}`
    );
